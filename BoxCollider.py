...
class BoxCollider:
    ...
    def check_collision(self, other, single_point=False):
        ...
        vertices = other_collider.vertices()
        for ver in vertices:
            start = other_collider.parent.position.to_np()
            end = ver
            vector = end - start
            norm = np.linalg.norm(vector)
            hit = self.Raycast(start, vector / norm, maxDistance=norm)
            if hit.point is not None:
                V = Vector3(tuple(
                    hit.point)) - self.parent.position  #
                if V.dot(collision_axis) > 0:
                    collision_axis = -collision_axis
                depth = (Vector3(tuple(hit.point)) - Vector3(tuple(end))).dot(-collision_axis)
                hits.add((tuple(end), collision_axis, depth))

        vertices = self.vertices()
        for ver in vertices:
            start = self.parent.position.to_np()
            end = ver
            vector = end - start
            norm = np.linalg.norm(vector)
            hit = other_collider.Raycast(start, vector / norm, maxDistance=norm)
            if hit.point is not None:
                V = Vector3(tuple(
                    hit.point)) - other_collider.parent.position
                if V.dot(collision_axis) > 0:
                    collision_axis = -collision_axis
                depth = (Vector3(tuple(hit.point)) - Vector3(tuple(start))).magnitude()
        ...
        #     if hit.point is not None:
        #         V = Vector3(tuple(
        #             hit.point)) - self.parent.position  # i think that maybe i need to add if collision_type == b then use self.other_collider.position
        #         if V.dot(collision_axis) > 0:
        #             collision_axis = -collision_axis
