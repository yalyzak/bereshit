...
class readData:
    ...
    def Update(self, dt):
        ...
        with self.lock:
            if keyboard.is_pressed('r'):
                self.base_quat = Quaternion(self.qJ, self.qI, self.qK, self.qReal)
    ...
    def Restart(self):
        """Restarts the thread and reopens the serial port"""
        print("[readData] Restarting...")

        # Stop the current thread and close the port
        self.Stop()

        # Wait a short moment to ensure thread and port are closed
        time.sleep(0.5)
        ...
        try:
            ...
            # Start reading again in a new thread
            thread = threading.Thread(target=self._read_loop, daemon=True)
            thread.start()
            print("[readData] Restarted successfully")

        except Exception as e:
            print(f"[readData] Failed to restart: {e}")
