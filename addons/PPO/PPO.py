...
class Agent:
    def __deepcopy__(self, memo):
        ...
        return new_agent
    def __init__(self, obs_dim, action_dim_discrete=0,action_dim_continuous=0, name="Agent", max_steps=100,optimizer=None,model=None, behaviorType= "default", save_dir="", update_rate=100):
    ...
    def end_episode(self):
        if Agent.episode % self.UPDATE_INTERVAL == 0:
            torch.save(self.model.state_dict(), self.dir)
            print(f"Episode {Agent.episode} ✅ model updated and saved")
        Agent.episode += 1

        self.memory.extend(self.episode_buffer)
        self.episode_buffer.clear()
        self.current_step = 0
        if self.behaviorType == "default":
            self.update()
        elif self.behaviorType == "heuristic":
            self.update()
        elif self.behaviorType == "inference":
            pass
