...
class BoxCollider:
    ...
    def check_collision(self, other):
        ...
        def generate_face_to_face_contact(ref_center, ref_axes, ref_half, inc_center, inc_axes, inc_half, normal_axis,
            ...
            # Want outward normal of incident face to be opposite collision_normal → dot(n_inc, collision_normal) should be negative.
            # So pick the face whose outward normal has negative dot with collision_normal:
...
class Object:
    ...
    def Stage1(self, children):
        ...
        # if self.Rigidbody.useGravity:
        #     gravity = Vector3(0, -9.8, 0)  # m/s^2
        #     self.Rigidbody.velocity += gravity * dt  # Impulse-like velocity change
    ...
    def apply_friction_impulse(self, contact, n, Jn_delta):
        """
        Friction impulse for one contact after you've applied a normal impulse delta (Jn_delta).
        Uses accumulated tangential impulse with Coulomb cone: |Jt_acc| <= mu * Jn_acc.
        Requires per-contact state in `contact`:
            contact["Jn_acc"]  : accumulated normal impulse (scalar)
            contact["Jt_acc"]  : accumulated tangential impulse vector (world)
            contact["t"]       : last tangent dir (world, unit) for stability
        """
        ...
        # Kinematic early out
        if (not rb1 or rb1.isKinematic) and (not rb2 or rb2.isKinematic):
            return

        # Ensure per-contact accumulators exist
        Jn_acc = contact.get("Jn_acc", 0.0)
        Jt_acc = contact.get("Jt_acc", Vector3(0, 0, 0))
        t_prev = contact.get("t", None)

        # Convenience accessors (world)
        v1 = rb1.velocity if rb1 and not rb1.isKinematic else Vector3(0, 0, 0)
        w1 = rb1.angular_velocity if rb1 and not rb1.isKinematic else Vector3(0, 0, 0)
        v2 = rb2.velocity if rb2 and not rb2.isKinematic else Vector3(0, 0, 0)
        w2 = rb2.angular_velocity if rb2 and not rb2.isKinematic else Vector3(0, 0, 0)

        # r vectors from COM (must be COM!)
        c1 = rb1.position if rb1 else p
        c2 = rb2.position if rb2 else p
        r1 = p - c1
        r2 = p - c2

        # Make sure inverse inertia is in WORLD space
        def Iinv_world(rb):
        ...
        v_rel = (v1 + w1.cross(r1)) - (v2 + w2.cross(r2))

        # Build stable tangent direction
        vt = v_rel - n * v_rel.dot(n)
        vt_mag = vt.magnitude()

        if vt_mag > 1e-7:
            t = vt / vt_mag
        else:
            # fallback to previous tangent if available
            if t_prev is not None and t_prev.magnitude() > 0:
                t = t_prev.normalized()
            else:
                # make any orthonormal to n
                a = Vector3(1, 0, 0) if abs(n.x) < 0.9 else Vector3(0, 1, 0)
                t = (a - n * a.dot(n)).normalized()
        ...
        # Effective mass k_t = (1/m1 + 1/m2) + (r1×t)^T I1 (r1×t) + (r2×t)^T I2 (r2×t)
        def ang_term(Iinv, r, dir_t):
            rx = r.cross(dir_t)
            # Iinv is a 3x3; convert to np only for the multiply; your Vector3 has helpers
            v = Vector3.from_np(Iinv @ rx.to_np())
            return rx.dot(v)

        k_t = 0.0
        if rb1 and not rb1.isKinematic:
            k_t += 1.0 / rb1.mass + ang_term(I1, r1, t)
        if rb2 and not rb2.isKinematic:
            k_t += 1.0 / rb2.mass + ang_term(I2, r2, t)
        ...
        # Desired unconstrained delta tangential impulse (scalar along t)
        # Use the current relative velocity *after* your normal impulse update
        vt_rel = v_rel.dot(t)
        dJt_unc = - vt_rel / k_t

        # Compute new accumulated candidate and cone-clamp:
        Jn_acc_new = Jn_acc + Jn_delta
        Jt_acc_new = Jt_acc + t * dJt_unc

        # Coulomb cone: ||Jt_acc_new|| <= mu * Jn_acc_new  (use accumulated normal impulse!)
        cone = mu * max(0.0, Jn_acc_new)  # guard negatives
        Jt_len = Jt_acc_new.magnitude()
        if Jt_len > cone:
            if Jt_len > 1e-12:
                Jt_acc_new = Jt_acc_new * (cone / Jt_len)
            else:
                Jt_acc_new = Vector3(0, 0, 0)

        # Apply only the delta from previous
        dJt = Jt_acc_new - Jt_acc
        if dJt.magnitude() < 1e-12:
            contact["Jn_acc"] = Jn_acc_new
            contact["Jt_acc"] = Jt_acc_new
            contact["t"] = t
            return

        # Apply to bodies
        if rb1 and not rb1.isKinematic:
            rb1.velocity += dJt / rb1.mass
            # rb1.angular_velocity += Vector3.from_np(I1 @ (r1.cross(dJt)).to_np())

        if rb2 and not rb2.isKinematic:
            rb2.velocity -= dJt / rb2.mass
            # rb2.angular_velocity -= Vector3.from_np(I2 @ (r2.cross(dJt)).to_np())

        # Store accumulators & stable tangent
        contact["Jn_acc"] = Jn_acc_new
        contact["Jt_acc"] = Jt_acc_new
        contact["t"] = t
