def run_renderer(root_object):
    ...
    import math
    import bereshit  # Your custom object library
    from bereshit import Vector3D

    camera = root_object.search_by_component("camera")

    # === Constants ===
    WIDTH, HEIGHT = camera.camera.width, camera.camera.hight
    FOV = camera.camera.FOV
    VIEWER_DISTANCE = camera.camera.VIEWER_DISTANCE
    ...
    def render_hierarchy(canvas, servo, parent_pos, parent_rot):
        if isinstance(servo, bereshit.Servo):
            obj = servo.obj
        else:
            obj = servo
    ...
    def update_camera():
        # root_object.get_children_bereshit()[0].gravity(collidable_bereshit=[root_object.get_children_bereshit()[1]])
        # root_object.get_children_bereshit()[1].gravity()
