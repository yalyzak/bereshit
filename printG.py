...
class BereshitRenderer(moderngl_window.WindowConfig):
    ...
    def on_render(self, time: float, frametime: float):
        ...
        cam_rot = self.camera_obj.rotation
        forward = rotate_vector_old(Vector3(0, 0, 1), Vector3(0, 0, 0), cam_rot).to_np()
        ...
        for item in self.meshes:
            ...
            # Simple model matrix: scale * rotation * translation
            trans = Matrix44.from_translation(pos)
            scale = Matrix44.from_scale(size * 0.5)
            model = trans @ scale  # Skip rotation for now or convert to quaternion later
